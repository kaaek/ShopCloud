locals {
  ...
  frontend_sites = {
    customer = {
      ...
      origin_domain = var.customer_ingress_domain_name
    }
  }
...
resource "aws_wafv2_web_acl" "cloudfront" {
  ...
  name        = var.waf_name
  description = "CloudFront protection for ShopCloud customer path"
  scope       = "CLOUDFRONT"
...
resource "aws_cloudfront_origin_request_policy" "forward_all_headers" {
  ...
  comment = "Forward viewer headers, cookies, and query strings to the public ingress"
...
resource "aws_cloudfront_distribution" "customer" {
  ...
  comment         = "ShopCloud customer frontend"
  aliases         = [local.root_domain_name, local.customer_alias_name]
  price_class     = var.cloudfront_price_class
  web_acl_id      = aws_wafv2_web_acl.cloudfront.arn

  origin {
    domain_name = var.customer_ingress_domain_name
    origin_id   = "shopcloud-customer-origin"
